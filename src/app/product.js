import {
  ...
  export const createProductCard = ({
    ...
    card.innerHTML = `
     <div class="w-full text-neutral-700">
        <img
          src="${image}"
          class="h-36 ms-5 -mb-16 product-img"
          alt=""
        /> 
        <div class="product-info border border-neutral-700 w-full p-5 pt-20 ">
          <p class="text-lg line-clamp-1 font-bold mb-5">${title}</p>
          ...
          <div class="rating flex justify-between border-b border-neutral-700 pb-5 mb-5">
            <div class="flex rating-start">${ratingUI(rate)}
            </div>
            <p class="rating-text">( ${rate}/${count} )</p>
          </div>
          <div class="font-bold mb-2 text-xl">$ <span>${price}</span></div>
          <button ${
            isCartItem && "disabled"
          } class=" add-to-cart-btn border w-full block p-3 border-neutral-700 ${
      isCartItem && "bg-neutral-700 text-white"
    }">${isCartItem ? "Added" : "Add to Cart"}</button>
              </div>
            </div>
     `;
  
    const addToCart = card.querySelector(".add-to-cart-btn");
    addToCart.addEventListener("click", addToCardBtnHandler);
  ...
  const addToCardBtnHandler = (event) => {
    ...
    totalAmount.innerText = calculateRecordCostTotal();
